...
@Schema(title = "Database Security provider", description = "Fill only the columnName's ,tableName and modelName. propertyName's and entityName are not necessary.")
public class DatabaseProviderConfig extends AbstractProviderConfig {

    public static final String DATABASE = "DATABASE";

    @NonProfilizableProperty(value = "${security.providers.database.modelName}", description = "WaveMaker DB service name for user authentication")
    @NotBlank
    ...
    @JsonPropertyDescription("When useRolesQuery is false, then you can use this property to retrieve roles.")
    private String roleColumnName;

    @JsonPropertyDescription("Set to true when want to use custom db query to retrieve roles, false when want to depend on basic query generated by WaveMaker. This basic query is generated by using the roleColumnName")
    private boolean useRolesQuery;

    @NonProfilizableProperty(value = "${security.providers.database.rolesByUsernameQuery:null}", description = "SQL/HQL query returning roles for a username")
    @JsonPropertyDescription("If useRolesQuery is true then provide the db query to retrieve roles")
    ...
    @NonProfilizableProperty(value = "${security.providers.database.queryType:HQL}", description = "Query language for user lookup",
        supportedValues = {"HQL", "NATIVE_SQL"})
    @JsonPropertyDescription("If useRolesQuery is true, set the queryType to HQL or NATIVE_SQL based on the db query passed in rolesByUsernameQuery property")
    private String queryType;

    @NonProfilizableProperty(value = "${security.providers.database.usersByUsernameQuery}", description = "SQL/HQL query returning user record by username")
